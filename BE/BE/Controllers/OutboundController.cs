...
namespace BE.Controllers
{
    ...
    public class OutboundController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetReceipts()
        {
            ...
            {
                Id = r.Doid,
                Code = string.IsNullOrEmpty(r.Docode) ? $"PX{r.Doid.ToString().PadLeft(4, '0')}" : r.Docode,
        ...
        public async Task<IActionResult> CreateReceipt([FromBody] OutboundReceiptDto req)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(delivery.Docode))
                {
                    delivery.Docode = $"PX{delivery.Doid.ToString().PadLeft(4, '0')}";
                    _context.SalDeliveries.Update(delivery);
                }
                ...
                foreach (var item in req.Items)
                {
                    _context.SalDeliveryLines.Add(new SalDeliveryLine
                    {
                        ...
                        DeliveredQty = Convert.ToInt32(item.Qty),
                        Price = item.Price,
        ...
        public async Task<IActionResult> UpdateReceipt(int id, [FromBody] OutboundReceiptDto req)
        {
            ...
            try
            {
                ...
                if (delivery == null || delivery.Status != "pending") return BadRequest(new { message = "Chỉ được sửa phiếu đang chờ duyệt!" });
        ...
        public async Task<IActionResult> CompleteReceipt(int id)
        {
            ...
            try
            {
                foreach (var item in delivery.SalDeliveryLines)
                {
                    ...
                    if (qtyProp != null)
                    {
                        var existingStock = await _context.WmsStockBalances.FirstOrDefaultAsync(s => s.VariantId == item.VariantId && s.LocationId == locId);
            ...
            catch (Exception ex) { await transaction.RollbackAsync(); return BadRequest(new { message = "LỖI SQL: " + (ex.InnerException?.Message ?? ex.Message) }); }
        }
    }
