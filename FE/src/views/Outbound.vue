      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
          <div class="px-6 py-4 border-b flex items-center justify-between bg-gray-50 shrink-0">
            <h3 class="text-lg font-bold flex items-center gap-2"><DocumentArrowUpIcon class="w-6 h-6 text-primary-600"/> {{ modalMode === 'add' ? 'Lập Phiếu Xuất Kho' : `Chi tiết: ${formData.code}` }}</h3>
            ...
            <form id="outboundForm" @submit.prevent="handleSubmit" class="space-y-6">
              <div class="bg-slate-50 p-4 rounded-lg border border-slate-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div><label class="block text-xs font-bold mb-1">Ngày xuất *</label><input v-model="formData.date" :disabled="modalMode === 'view'" type="date" required class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-primary-500 disabled:bg-gray-100"></div>
                  <div><label class="block text-xs font-bold mb-1">Trạng thái</label>
                    <select v-model="formData.status" :disabled="modalMode === 'view'" class="w-full border rounded-lg px-3 py-2 text-sm font-semibold disabled:bg-gray-100" :class="getStatusBadge(formData.status).class">
                      <option value="pending">Chờ duyệt</option><option value="shipping">Đang giao hàng</option><option value="completed">Đã giao thành công</option>
                    </select>
                  </div>
                </div>
                <div class="mt-4">
                  <label class="block text-xs font-bold mb-1">Khách hàng nhận *</label>
                  <select v-model="formData.customerId" :disabled="modalMode === 'view'" required class="w-full border rounded-lg px-3 py-2 text-sm focus:ring-primary-500 disabled:bg-gray-100 cursor-pointer">
                    <option value="" disabled>-- Chọn Khách hàng --</option>
                    <option v-for="cus in mockCustomers" :key="cus.id" :value="cus.id">[{{ cus.id }}] {{ cus.name }}</option>
                  </select>
                </div>
              </div>

              <div>
                <h4 class="text-sm font-bold mb-2">Danh sách Hàng hóa Xuất</h4>
                <div v-if="modalMode === 'add'" class="flex flex-col sm:flex-row gap-2 mb-3 bg-blue-50 p-3 rounded-lg border border-blue-100">
                  <select v-model="selectedSkuToAdd" class="flex-1 border rounded-lg px-3 py-2 text-sm focus:ring-primary-500 cursor-pointer">
                    <option value="" disabled>-- Chọn Sản phẩm xuất --</option>
          ...
          <div class="px-6 py-4 border-t flex flex-col sm:flex-row justify-between gap-3 shrink-0">
            ...
            <div class="flex gap-2 w-full sm:w-auto">
              ...
              <button v-if="modalMode === 'add'" type="submit" form="outboundForm" class="flex-1 sm:flex-none px-5 py-2.5 bg-primary-600 text-white rounded-lg text-sm font-bold hover:bg-primary-700 shadow-md"><DocumentArrowUpIcon class="w-5 h-5 inline mr-1"/> Hoàn tất Phiếu</button>
            </div>
          </div>
        </div>
      </div>
