...
const formData = ref({ 
  id: null, code: '', date: '', expectedDate: '', supplierId: '', 
...
const openModal = (mode, po = null) => {
  ...
  if (po) {
    formData.value = JSON.parse(JSON.stringify(po)) 
  } else {
    formData.value = { id: null, code: '', date: getToday(), expectedDate: getToday(), supplierId: '', items: [], note: '', status: 'draft' }
  }
...
const getStatusBadge = (status) => {
  switch(status) {
    ...
    case 'sent': return { text: 'Đã gửi NCC', class: 'bg-blue-100 text-blue-700 border-blue-300' }
  ...
  <div class="space-y-5 md:space-y-6 animate-fade-in pb-10 px-0 md:px-1 relative">
    ...
    <div class="bg-white p-3 md:p-4 rounded-xl border border-gray-200 flex flex-col sm:flex-row items-center gap-3 md:gap-4 shadow-sm">
      ...
      <select v-model="filterStatus" class="w-full sm:w-auto border border-gray-200 rounded-lg text-sm px-4 py-2 outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer">
        ...
        <option value="sent">Đã gửi NCC</option><option value="completed">Hoàn thành</option>
      </select>
    </div>
          ...
          <tbody class="divide-y divide-gray-100 bg-white">
            ...
            <tr v-for="po in filteredPOs" :key="po.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-5 py-3 text-right space-x-2 whitespace-nowrap">
                <button v-if="po.status === 'draft' || po.status === 'pending'" @click="openModal('edit', po)" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded" title="Sửa Đơn"><PencilSquareIcon class="w-5 h-5" /></button>
                <button @click="openModal('view', po)" class="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded" title="Chi tiết"><EyeIcon class="w-5 h-5" /></button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <select v-model="formData.status" :disabled="modalMode === 'view'" class="w-full border rounded-lg px-3 py-2 text-sm font-semibold disabled:bg-gray-100" :class="getStatusBadge(formData.status).class">
                      <option value="draft">Bản nháp</option><option value="pending">Chờ trưởng phòng duyệt</option><option value="sent">Đã gửi cho NCC</option><option value="completed">Đã nhận hàng (Hoàn thành)</option>
                    </select>
                  </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label class="block text-xs font-bold mb-1">Ngày lập phiếu *</label><input v-model="formData.date" :disabled="modalMode === 'view'" type="date" required class="w-full border rounded-lg px-3 py-2 text-sm disabled:bg-gray-100"></div>
                  <div><label class="block text-xs font-bold mb-1">Ngày dự kiến giao *</label><input v-model="formData.expectedDate" :disabled="modalMode === 'view'" type="date" required class="w-full border rounded-lg px-3 py-2 text-sm disabled:bg-gray-100"></div>
                </div>
              </div>
                  ...
                  <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                      ...
                      <tr v-for="(item, idx) in formData.items" :key="idx" class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 text-right"><input v-if="modalMode !== 'view'" v-model.number="item.qty" type="number" min="1" class="w-full text-right border rounded px-2 py-1 font-bold text-indigo-700"><span v-else class="font-bold text-indigo-700">{{ item.qty }}</span></td>
                        <td class="px-4 py-2 text-right"><input v-if="modalMode !== 'view'" v-model.number="item.price" type="number" min="0" class="w-full text-right border rounded px-2 py-1"><span v-else>{{ formatCurrency(item.price) }}</span></td>
                    ...
                    <tfoot class="bg-gray-50 font-bold border-t"><td colspan="3" class="px-4 py-3 text-right uppercase">Tổng cộng PO:</td><td class="px-4 py-3 text-right text-indigo-700">{{ totalQty }}</td><td></td><td class="px-4 py-3 text-right text-emerald-600">{{ formatCurrency(totalPrice) }}</td><td v-if="modalMode !== 'view'"></td></tfoot>
                  </table>
                </div>
              </div>
          ...
          <div class="px-6 py-4 border-t flex flex-col sm:flex-row justify-between gap-3 bg-white shrink-0">
            <div class="flex gap-2 w-full sm:w-auto"><button v-if="modalMode !== 'add'" @click="handlePrintPO" class="flex-1 sm:flex-none px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-bold border flex items-center justify-center gap-2"><PrinterIcon class="w-5 h-5"/> In Đơn Đặt Hàng</button></div>
